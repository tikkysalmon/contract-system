...
// รอตรวจสอบ — (2026-09-06 เพิ่มใหม่) ลูกค้าส่งฟอร์มกลับมาแล้ว แต่ทีมเร่งรัดหนี้สินยังไม่ได้กดปุ่ม "ยืนยัน"
//     ตรวจสอบข้อมูล (reviewed_at) เลยยังบอกไม่ได้ว่าเรียบร้อยจริงไหม
// 1.3 สัญญาไม่เรียบร้อย — auto เมื่อทีมเร่งรัดหนี้สินปฏิเสธ/ขอแก้ไขข้อมูล (rejected_at ไม่ null) — มาก่อน "รอ
//     ตรวจสอบ" เสมอ (ถ้าปฏิเสธไปแล้วไม่ต้องพะวงว่ายังไม่ตรวจสอบ)
// 1.4 สัญญาลูกค้าเรียบร้อย — auto เมื่อทีมเร่งรัดหนี้สินกด "ยืนยัน" แล้วว่าข้อมูลถูกต้องไม่ต้องแก้ไข (reviewed_at)
//     และยังไม่ถึงขั้นตอนแพ็คกิ้ง (ยังไม่มี IMEI+Serial) — ไม่เกี่ยวกับลายเซ็นพนักงาน ("เซ็นเอกสาร" เป็นคนละ
//     action กับ "ยืนยัน" นี้ จะเซ็นเมื่อไหร่ก็ได้)
// รอพนักงานเซ็นเอกสาร — auto เมื่อผ่านการยืนยันแล้ว (reviewed_at) และทีมแพ็คกิ้งกรอก IMEI+Serial ครบแล้ว
//     (พร้อมเป็น "เสร็จสมบูรณ์") แต่ตรวจพบว่ายังไม่มีลายเซ็นพนักงาน — กันเคสสัญญาค้างเซ็นเงียบๆ โดยไม่มีใครสังเกต
// 1.5 สัญญาเสร็จสมบูรณ์ — auto เมื่อผ่านการยืนยันแล้ว มี IMEI + Serial Number ครบ และลายเซ็นพนักงานครบถ้วน
function computeContractStatus(input) {
  ...
  if (!input.reviewedAt) return { key: 'pending_review', label: 'รอตรวจสอบ' };
